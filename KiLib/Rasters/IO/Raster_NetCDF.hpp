...
*/ 
template<typename T>
static KiLib::Rasters::Raster<T> FromNetCDF(const std::string &path, const std::string var_name, std::function<bool(T&, size_t, double)> construct_val) {
   ...
   for( size_t x = 0; x < dim_x_len; x++ ) {
      for( size_t y = 0; y < dim_y_len; y++ ) {
         ...
         T v;
         for( size_t t = 0; t < dim_time_len; t++ ) {
            size_t indexp[] = { t, y, x };
            double result;
