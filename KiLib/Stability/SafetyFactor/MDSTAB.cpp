   ...
   double l) const
{
   ...
   const double Frl = 0.5 * K0 * (gamma_s - gamma_w * pow(m, 2)) * l * pow(z, 2) * ctheta * tphi + Crl * l * z * ctheta;

   // Rankine solution for cohesive soils
   const double K = std::max(
      0.0, 4.0 * pow(ctheta, 2) * (pow(ctheta, 2) - pow(cphi, 2)) + (4 * pow(Crl / (gamma_s * z), 2) * pow(cphi, 2)) +
              (8 * (Crl / (gamma_s * z)) * pow(ctheta, 2) * sphi * cphi));

   // Passive (p) and active (a) earth pressure coefficients for Rankine solution for cohesive soils (Milledge Equation
   // 8)
   const double Kp = (1 / pow(cphi, 2)) * (2 * pow(ctheta, 2) + 2 * (Crl / (gamma_s * z)) * cphi * sphi + sqrt(K)) - 1;
   const double Ka = (1 / pow(cphi, 2)) * (2 * pow(ctheta, 2) + 2 * (Crl / (gamma_s * z)) * cphi * sphi - sqrt(K)) - 1;

   // Slope-parallel passive force on the upslope margin (Milledge Equation 12 modified)
   const double Fdu = 0.5 * Ka * pow(z, 2) * (gamma_s - gamma_w * pow(m, 2)) * w * cos(delta - theta);

   // Slope-normal passive force on the upslope margin (Milledge Equation 13 modified)
   const double Fnu = 0.5 * Ka * pow(z, 2) * (gamma_s - gamma_w * pow(m, 2)) * w * sin(delta - theta);

   // Slope-parallel passive force on the downslope margin (Milledge Equation 12)
   // const double Frd = 0.5 * Kp * pow(z, 2) * (gamma_s - gamma_w * pow(m, 2)) * w * cos(delta - theta);
   ...
   const double Fnd = 0.5 * Kp * pow(z, 2) * (gamma_s - gamma_w * pow(m, 2)) * w * sin(delta - theta);

   // Weight of block (see Milledge Equation 14)
   const double Fnc = (gamma_s - gamma_w * m) * z * pow(ctheta, 2) * w * l;
   ...
   const double FS = (Frb + 2 * Frl + Frd - Fdu) / Fdc;
